...
int getRemoveEnemy(int index1, int index2, int index3) {
	...
	for (int i = 0; i < N; i++) {
		...
		if (p1.cnt == -1) break;
			...
			if (map[p1.x][p1.y] && getDist(pos1.first, pos1.second, p1.x, p1.y) <= D) {
				map[p1.x][p1.y] = 0;
				ret++;
			}
			if (map[p2.x][p2.y] && getDist(pos2.first, pos2.second, p2.x, p2.y) <= D) {
				map[p2.x][p2.y] = 0;
				ret++;
			}
			if (map[p3.x][p3.y] && getDist(pos3.first, pos3.second, p3.x, p3.y) <= D) {
				map[p3.x][p3.y] = 0;
				ret++;
			}
